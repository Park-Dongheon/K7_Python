...
#숙제: step1-당첨번호 생성, step2-100장 복권발행할때 당첨 확인처리
#100장 복권 발행(list로 저장 후 정렬), 보너스 번호 생성(list로 저장 후) -> 복권+보너스 list 저장 후 -> 당첨비교 + 당첨 확인 함수로 구현

import random

# 두 세트 생성, 값만 가지고 있음(두 세트의 값이 동일)
set1 = {1, 2, 3, 4, 5}
set2 = {3, 2, 1, 6, 7}
...
# set1.intersection
print(len(set1.intersection(set2)) == 3)      # {1, 2, 3}, se1과 set2의 교집합 부분 출력, ==3 개수를 비교함-True/False 반환
# 로또 복권 번호 생성 함수
def generate_lotto_numbers():
    ...
    lotto_numbers = set(random.sample(numbers, 6))      # sample(): numbers에서 6개를 뽑음
...
for idx in range(1, 101):
    ...
    elif len(lotto_numbers.intersection(winning_numbers)) == 5:                         # intersection() 함수:
